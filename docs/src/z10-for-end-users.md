...
DataDeps.jl tries to have very sensible defaults.
 ...
 - `DATADEPS_PROGRESS_UPDATE_PERIOD` -- how often (in seconds) to emit download progress updates
    - This is used by the default `fetch_method` to throttle progress reporting.
    - With the built-in `fetch` method, the same interval controls progress callback invocation and progress logging.
    - `Inf` disables progress updates.
    - default: `5` (seconds) usually; `Inf` if `DATADEPS_ALWAYS_ACCEPT` is set.
