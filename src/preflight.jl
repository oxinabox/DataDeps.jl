...
"""
    preflight(datadep, local_filepath[s])::Bool)

Peforms "preflight" checks on the local files without having to download them.
This is tool for creating or updating DataDeps,
allowing the author to check the files e.g. before they are uploaded.
Preflight checking includes checking the checksum, and the making sure the `post_fetch_method` runs without errors.
...
Returns: true or false, depending on if the preflight checks were all good, or not.
...
"""
function preflight(dd::DataDep, local_filepath)
...
function preflight(datadep_name::AbstractString, fetched_files)
    return preflight(registry[datadep_name], fetched_files)
end
